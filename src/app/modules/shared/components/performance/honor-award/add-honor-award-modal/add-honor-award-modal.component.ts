...
@Component({
	selector: 'app-add-honor-award-modal',
	templateUrl: './add-honor-award-modal.component.html'
})

export class AddHonorAwardModalComponent implements OnInit {
	...
	ngOnInit() {
		...
		.subscribe(
			...
			err => {
				console.error('SA.honorAwardHistory.honorAwardHistory-form.component  - getGrades', err);
			}
		);
		...
		this.studentService.getStudentById(this.student_id).subscribe(
			...
			err => {
				console.error(
					'SA.honorAwardHistory.honorAwardHistory-form.component - getUser',
					err
				);
			}
		);
	...
	get formModel() {
		const composition_title = this.form.get('composition').value
	...
	submitForm() {
		if (!this.loading) {
			...
			if (this.form.valid && fm.event) {
						this.haService
						.createStudentHA(this.student_id, new HonorAwardModel(fm))
						.subscribe(
							(response: any) => {
								...
								this.feedback = 'Honors and Awards information added';
								//this.form.reset();
							...
							err => {
								...
								this.feedback =
									'Unable to add honors and awards information';
