...
import { CatalogService, GoogleAnalyticsEventsService, UserModel } from '../../../../../shared/shared.module';

@Component({
	selector: 'app-add-private-study-modal',
	templateUrl: './add-private-study-modal.component.html'
})

export class AddPrivateStudyModalComponent implements OnInit {
	...
	ngOnInit() {
		...
		.subscribe(
			...
			err => {
				console.error('SA.privateStudyHistory.privateStudyHistory-form.component - getStudySubjects', err);
			}
		);
	...
	submitForm() {
		...
		this.googleAnalyticsEventsService.emitEvent('Public', 'Form Submition', 'Private Studies Form', 20040);
		this.requestFailed = this.requestSuccess = false;

		if (this.form.valid) {
			this.psService.createStudentPrivateStudy(this.student_id, new PrivateStudyModel(this.formModel))
				.subscribe(
					(response: any) => {
						
						this.student = response.data;
						this.googleAnalyticsEventsService.emitEvent('Public', 'Form Submition', 'Private Study History Form', 19100);
						this.feedback = 'Private study information added';
						 //this.form.reset();
					...
					err => {
						this.loading = false;
						console.error('SA.extraCurricular.privateStudies-form.component - putUserPrivateStudies', err);
						this.googleAnalyticsEventsService.emitEvent('Public', 'Form Submition', 'Private Studies Form', 20240);
						this.feedback = 'Unable to add private study information';
	...
	isFieldInvalid(fieldName: string) {
		...
		this.googleAnalyticsEventsService.emitEvent('Public', 'Form Submition', 'Private Studies Form FormField: ' + this.form.get(fieldName), 20340);
